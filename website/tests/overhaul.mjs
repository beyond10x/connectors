...
try {
  ...
  await visit('/adapters/kubernetes/contracts/reads');
  assert.equal(await navigation.locator('[aria-current="page"]').innerText(),'Kubernetes reads');

  await visit('/search?q=Kubernetes&owner=kubernetes');await results();
  assert.equal(await page.getByLabel('Owner',{exact:true}).inputValue(),'kubernetes');
  const resultRoutes=()=>page.locator('.search-results>li a').evaluateAll(links=>links.map(link=>link.getAttribute('href')).sort());
  assert((await resultRoutes()).includes('/adapters/kubernetes'),'the guide is among the results');
  assert(await page.locator('.search-results mark').count()>0,'excerpts highlight the query');
  const guide=page.locator('.search-results').getByRole('link',{name:'Kubernetes',exact:true});
  assert.equal(await guide.getAttribute('href'),'/adapters/kubernetes');
  await guide.click();await page.waitForURL(base+'/adapters/kubernetes');
  await page.goBack({waitUntil:'networkidle'});await results();
  assert.equal(await page.getByLabel('Search all documentation',{exact:true}).inputValue(),'Kubernetes');
  await page.getByLabel('Document type',{exact:true}).selectOption('Contract');await results();
  for(const route of await resultRoutes())assert(route.startsWith('/adapters/kubernetes/contracts/'),route);
  ...
  assert.equal(await page.locator('.search-results>li').count(),8);
