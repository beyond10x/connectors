...
A v1alpha2 service answers four questions separately, as `docs/design.md` § 6.2 requires: implemented by this artifact, enabled in this instance, ready with its dependencies, **authorized for this caller**. The legacy binding describes implemented/enabled operations and checks dependencies on invocation; it has no per-person policy projection or readiness grant in the descriptor. The fourth needs a stable caller. Static bearer identifies one receiver-configured service principal, not the individual people sharing its bytes; it cannot provide individual caller isolation.
...
Under **all three admission profiles**, operations are the caller-visible projection of implemented, fully bound, enabled and currently metadata-policy-admitted operations. Static bearer evaluates an explicit local policy for its one configured service principal; it does not distinguish people sharing that credential. Reserved, invalidly declared, unsupported-codec or unbound operations are omitted even if a package/vocabulary names them. Describe uses admitted host metadata and performs no provider probe, credential refresh, approval spending or business dispatch. Dependency readiness and possession of a current one-shot approval do not determine operation visibility. A not-ready connection can therefore retain useful admitted operation metadata and separately admitted management repair operations.

#### 3.2.1 Visibility, lookup and refusal precedence (E12)

The public projection is not the receiver's private implementation registry. Extended invocation may use that registry to distinguish an implemented-but-disabled operation from an absent/unbound one **only after current policy admits lookup in that exact operation/target scope**. Lookup admission is a receiver policy decision, not a new public permission flag, caller assertion or bypass to a handler. Denial/unavailable authority returns a safe error without exposing whether an implementation, key, attempt, approval or result exists. If policy does not admit lookup of an unknown spelling, not_granted wins; not_found is observable only within a lookup scope the caller is allowed to inspect. No public disabled descriptor field is introduced.

For a valid selected envelope, apply this order: (1) transport/framing/authentication/version and syntactic identifier/size checks; (2) current host operation/lookup/result-scope policy and verified executor binding, including current host connection/resource observation authority; (3) exact current descriptor projection revision; (4) private implemented/bound lookup and enablement; (5) operation input shape and confirmation that the validated input remains inside the admitted scope; (6) mutation key observation, if keyed; (7) only for a candidate new attempt, required approval, dependency/credential/provider-permission preflight and the existing audit/attempt/spend/dispatch sequence. Step 2 may consult private host metadata and parse submitted target coordinates internally to decide current policy, but cannot disclose private lookup errors or an unadmitted target; absence/ambiguity is not authority to widen scope. Provider permission/readiness work belongs to step 7, not describe or replay. All relevant current host admission checks are repeated/fenced at the binding's dispatch boundary; a stale earlier descriptor or successful lookup is never a durable execution grant.

| State / condition | Describe | Invoke (current admitted lookup and revision unless stated otherwise) |
|---|---|---|
| Unknown, unimplemented, invalid/unbound or unsupported operation binding | omitted | not_found; no handler, provider, approval or key lookup |
| Implemented/bound but configuration-disabled | omitted | forbidden; receiver identifies it privately after admitted lookup, without dispatch or original-result disclosure |
| Current host metadata/lookup/operation policy denies caller or executor binding | omitted where metadata denied | not_granted; deny before stale/existence/key/approval disclosure |
| Required current host policy is unavailable | no success claiming an admitted projection | unavailable; no fallback to static policy or cached authority |
| Current host connection/resource observation scope denied | metadata only if its separate scope remains admitted | forbidden; no target-existence or mutation/result disclosure |
| Enabled and policy-admitted, but dependency/credential/provider permission not ready for a new attempt | visible | after key observation and candidate approval, selected connection_not_ready/insufficient_scope/route refusal/unavailable/forbidden as applicable; do not collapse global viability and exact operation eligibility |
| Enabled/admitted, missing a fresh execution approval | visible with approval requirement | a candidate new attempt gets approval_required; exact admitted replay neither requires nor spends fresh execution approval |
| Enabled/admitted with exact matching retained key, new-attempt dependencies unavailable | visible | original outcome/wait/quarantine observation under F02 current result admission; no provider preflight, marker generation or new approval spend |
| Enabled/admitted/ready with required new-attempt approval | visible | normal invocation under existing profile/audit/dispatch rules |
| Supplied revision is old, while current host lookup/observation authority still admits the request | current projection on describe | stale_description before private disabled/not_found/input/key decisions; caller must deliberately refresh description and resubmit |
| Same stale request also loses current host lookup/observation authority | governed by current metadata scope | current not_granted/forbidden/unavailable wins without stale/existence/outcome disclosure |

Every refusal before current result admission/key observation omits mutation metadata; absence does not mean not_attempted for a possibly existing original. A newly admitted candidate's later approval/preflight refusal can report not_attempted only under F01/F02's winner-recheck and dispatch-evidence rules. Descriptor freshness, current result authority, dependency readiness and approval are separate facts. Input validation cannot authorize a key lookup or expose a hidden operation schema before the prior admission steps.

The revision binds the selected codec, caller projection and effect-relevant configuration/meaning. Enable/disable, visible membership, input/output meaning or authority-binding changes invalidate affected descriptions; credential rotation or transient readiness loss alone need not change operation meaning. A caller with a current descriptor that omits a disabled operation can receive forbidden for an admitted guessed ID, while a caller presenting its old pre-disable revision receives stale_description first. Subsequent re-enable cannot authorize execution on a stale description. Invocation must re-evaluate current policy even immediately after describe.

Federation exposes only the selected gateway/leaf codec, binding, enablement and authorized metadata intersection, under the current delegated describe proof. An alias or broad downstream bearer cannot widen it. If the selected complete projection cannot establish a required current leaf admission, describe fails safely as unavailable rather than silently claiming cached policy is current; no provider probe is implied. Invoke independently admits gateway and leaf, resolves the fixed leaf meaning, and preserves refusal/unknown outcomes without fallback or resend. Legacy projection retains its separate restricted lookup rule below. These are future receiver/projection obligations; the current descriptor-vector implementation does not already provide this private extended lookup.
...
1. Select configured endpoint and fixed version path, bound framing and authenticate into AuthenticatedContext, then strictly decode the envelope. Apply §3.2.1 precedence: current trusted lookup/observation policy and executor binding before revision/existence disclosure; private bound/enablement lookup and operation admission before any key observation; candidate-only approval and dependency preflight before dispatch. No application input chooses tenant/principal/realm/authority or expands the destination.
...
Authenticated/admitted contexts remain host values whose trusted derivation belongs to their owner bindings. The [execution-audit owner](../audit.md) now selects AuditRecord's private host-qualified identity, one acknowledged anchor and at most one final observation, with optional references to existing attempt and Connection records. Its [ESS model](../../../ess/domains/execution_audit.yaml) declares those relations and lifecycle; actual storage acknowledgements, atomic append and trusted correlation remain implementation obligations. Grant configuration and SaaS tenant/principal ownership remain separately selected work. This document introduces no stub entity or inferred cardinality to satisfy validation.
...
Private `connectors.declarations.OperationAvailability` and `AvailabilityDecision` record selected availability facts and a projection/lookup decision. They are not public descriptor fields, policy assertions, persistent entities or a computed admission algorithm. ESS does not enforce §3.2.1, result authority, revision changes, federation intersection or any dispatch fence.
