...
The coordinator applied both supplied index/capability patches after merging the
media/vocabulary source. A further cross-check of the native authentication
sections found that Kubernetes and Docker also select `static_config` under
`auth.acquisition`, even though they need no interactive acquisition flow.
Their native contract tables and root index now include that dependency explicitly.
The root definitions also distinguish generic series semantics from native query
languages, list static_config activation, mark signing reserved/refused and link
the separately selected read-refresh retry binding.
