...
- [ ] **`specs/flux/core-v1.json`** is deleted together with every claim that something is emitted
      ...
      describes nothing.
- [ ] **`migration/`** is decided against its live successor: flux-roadmap decision 0024's
      ...
      fixtures.
- [ ] `SOURCES.toml`'s "known defects" header shrinks by exactly the entries closed, and the
...
- [ ] Each removal is its own reviewable commit with the reason in the body — a deleted 157 KB
