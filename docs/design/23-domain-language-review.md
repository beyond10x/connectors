...
**Status: analysis complete for review; proposed decisions, not approved design or implementation authority. Revised once after the independent naming review round one (findings NIR-001..003; see the checks section).**
...
| `connectors.event` | Event, Webhook, Subscription, Delivery | Keep the event domain; distinguish event receiver, webhook registration, consumer subscription and delivery. Some existing entities are declared only. Provider events only: Connectors-owned operational events are a separate family owned by runtime audit (see glossary, NIR-003). |
...
| **OperationalEvent** | A Connectors-owned runtime occurrence: a receiver health transition, endpoint lifecycle or revocation, session termination, credential expiry, delivery failure | Added for NIR-003. This retains the baseline's existing disjoint families: the design model and ESS already declare data and operational events with separate subscriptions, and the current data-event protocol explicitly excludes operational events ([S09]; `ess/system/domains/event.yaml` EventFamily; `crates/protocol/src/event.rs:1`). Event/`event_ref` in this glossary is the data family (the baseline's DataEvent); EventReceiver describes external data intake only. An OperationalEvent has no catalog EventType and no native/polled provenance requirement, and is not selected by EventSubscription. Operational publication and consumption remain declared-only where no runtime exists; nothing here requires building an operational-event stream. |
...
| EventReceiver → Event | A receiver is the recorded ingestion provenance; it can produce many Events. Provenance never decides consumer stream membership — that is the logical stream (Endpoint, EventType, source partition), D4/NIR-001 | Retention belongs to EventStore policy. Stopping/removing a receiver does not erase retained events or change their refs. Cross-transport deduplication is D4. |
...
| Endpoint → Session | Zero or many Sessions; each admitted Session has exactly one endpoint attribution and at most one controlling principal — bound at invocation for platform-initiated Sessions, at acceptance for offered ones (D7) | Endpoint revocation triggers session authority handling; an event or participant cannot change its endpoint by assertion. Session history follows retention. |
...
**Consumer stream membership after deduplication (NIR-001).** A consumer's selection is decided against the logical stream — Endpoint, EventType and source partition — never against ingestion provenance. Cross-receiver deduplication changes only which receiver is recorded as provenance; it must not change which consumers see the occurrence, and each consumer selection that matches receives its own delivery of the one retained occurrence. Two receivers whose specs declare the same logical stream for one Endpoint (Slack socket and Events API webhook carrying the same event set) feed the same retained occurrences, so a subscription written against either receiver selects all of them. A subscription form scoped to a receiver therefore means the logical streams that receiver's spec and configuration declare, not the subset of occurrences whose provenance happens to name it. Walkthrough: consumer A selects the socket receiver, consumer B selects the webhook receiver, and the webhook wins the intake race; one occurrence is retained with webhook provenance, and both A and B read it, because both selections name the same logical stream. Stopping the webhook receiver afterwards stops that intake path only; retained occurrences stay readable under both selections, and the socket receiver continues the stream. Receivers with disjoint source partitions (two ARI Stasis applications) remain distinct logical streams, and deduplication never merges across them. The baseline's per-channel read attribution ([S07], [S08]) supports current behavior only; this membership rule is the explicit change that broader deduplication requires.
...
**Controlling principal for an offered Session (NIR-002).** The proposed rule is state-dependent ownership with an authorized claim: an offered Session has zero controllers while it waits, and acceptance is the claim transition that binds the accepting principal as the controlling principal for the Session's remaining lifetime. The policy lives in the Endpoint's incoming-session configuration: it names which principals or applications are authorized accepters and the offer deadline, and it is reviewed configuration like AccessRoute, not runtime assertion. Every authorized accepter for the Endpoint may inspect a pending offer. An explicit rejection by an authorized accepter is terminal for the offer — there is one offered Session, not one copy per eligible application — so on a tenant-shared Endpoint with two eligible applications, either may claim it and either may end it; deployments wanting per-application arbitration configure disjoint Endpoints or a single accepter. Timeout and provider cancellation likewise produce a terminal Session that never had a controlling principal. Offer-admission authority (the listener and this configuration) is therefore distinct from established-session control (the bound principal); a participant cannot become the controlling principal by asserting it, and control is not reassigned mid-session — transfer, if ever needed, is a later reviewed capability. The alternative — a configured controlling principal assigned already at offer admission — was rejected because it forces one owner to be named even where several applications are legitimately eligible, recreating the ambiguity it removes. The section 4 rule reads accordingly: at most one controlling principal, bound at invocation for platform-initiated Sessions and at acceptance for offered ones.
...
All four previously listed unresolved areas now have proposed answers: ownership/cardinality in D1–D2 and the relationship table; voice identities in D6; event lifecycle in D3–D4; incoming SIP policy and its scope boundary in D7. Datasource scope and lifecycle/compatibility corrections are explicit in D5 and D8. The independent naming review's three findings are answered in this revision (see the checks section): consumer stream membership in D4, the offered Session's controlling principal in D7, and the OperationalEvent family in the glossary. No internal discussion is represented as operator approval.
...
### Independent naming review round one

An independent reviewer, working from the document at its committed hash, the requirements and the source baseline without the author's conversation history, returned **revise before acceptance** with three findings. The verdict summary is in `story:reconcile-connectors-domain-language` revision 9 and the verbatim report is `review-result:domain-language-independent-review-round-one`; the revisions below were checked against that full report.

| Finding | Response in this revision |
| --- | --- |
| NIR-001 (blocker) — consumer stream membership after cross-receiver deduplication | D4 now defines membership by logical stream (Endpoint, EventType, source partition), never by ingestion provenance, with one delivery per matching consumer selection and the two-consumer race walkthrough; the EventReceiver → Event relationship row carries the same rule. |
| NIR-002 (warning) — controlling principal for incoming offered Sessions | D7 now specifies state-dependent ownership: zero controllers on a pending offer, acceptance as the authorized claim transition, the Endpoint's incoming-session configuration as the policy home, and terminal rejection semantics on shared Endpoints; the section 4 Session row is qualified to match. |
| NIR-003 (warning) — omitted operational-event family | The glossary now retains the baseline's disjoint families: Event/`event_ref` is the data family, OperationalEvent stays runtime-owned and declared-only, outside EventType, EventReceiver and EventSubscription; the `connectors.event` disposition says so. |

The reviewer supported the target, datasource, receiver/session and legacy-boundary distinctions and found no reason for another public account selector. The completeness statements in this document are corrected by NIR-003: the earlier inventory accounted for provider-declared and ESS-declared concepts and did not name the baseline's operational-event family, which `docs/design/01-domain-model.md`, `ess/system/domains/event.yaml` and the data-event protocol already keep separate.
