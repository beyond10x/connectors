...
Serialize each binding's transaction and session completion claim. After prepare, recheck the same pending session, deadline, grant/authority and generation under the completion owner, before making a durable commit decision. Expiry or revocation before that decision wins: abort the batch and publish no credential/Connection completion. A durable commit decision made before the deadline wins: finish commit and metadata publication idempotently, then set Completed. The deadline is the authorization cutoff, not a promise that disk I/O finishes by then. The expiry worker cannot subsequently mark that committed decision Expired. An uncertain durable write is recovery-required; never report a false Completed or Expired merely to end the wait.
