...
The receiver declaration datasources.observe fixes grafana-datasources and the admitted configured Grafana source under [resource discovery §4.5](../../contracts/discovery/resources/v1alpha1/semantics.md#45-fixed-declarations-recognition-and-configured-source-identity). Input contains only limit/cursor; no caller profile, source origin or mapping overrides it. Known reviewed provider types carry declared recognition independently of nullable candidates; unknown plugins stay observable with both recognition and candidate null. An allowlist may remove candidacy without proving provider disappearance.
