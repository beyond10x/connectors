...
Persistent product state lives behind host-owned ports. Examples include connection metadata, active credential references, assignments, config revisions, subscription cursors, approval redemption, and required execution audit. Transient protocol state remains in adapters, including SIP dialogs, SQL connections, WebSocket state, timers, and in-flight provider requests. The [§31 ownership inventory](#31-host-persistence-ownership-and-atomicity) assigns the narrow logical ports and required atomic groups; these are specification obligations, not implemented stores.
...
## 31. Host persistence ownership and atomicity

This inventory consolidates the selected contracts' obligations for E28. Each row names one **logical port owner**, not a Rust trait, universal keyed-byte API, database choice or advertised implementation. A port can have multiple replicas only when they share its one authoritative state and required serialization. Separate rows distinguish state ownership even where an atomic group requires one underlying host metadata authority. Adapters own provider protocol interpretation and transient connections, not these product ledgers.

| Port owner | Sole owned state and narrow decisions | Required failure/retention handoff | Normative contract |
|---|---|---|---|
| CustodyPort | Scoped immutable sensitive versions; coherent bounded credential sets; write_new/read/delete | Required durable write acknowledgement precedes publication. Unknown write is no usable publication proof; orphan cleanup does not establish exchange outcome. Keep material needed by valid uses; deletion never erases refresh consumption | [custody §§3–5](../contracts/auth/custody/v1alpha1/semantics.md) |
| ConnectionAuthorityPort | Stable connection/owner/fixed target, enabled/revoked facts, active generation/version reference, semantic revision and private publication fence | Serialize terminal revoke, replacement, publication and final dispatch admission. Readiness is a current fact reduction, not an independently writable status. Unknown metadata refuses; provider revoke outcome is separate | [connection §4](../contracts/auth/connection/v1alpha1/semantics.md) |
| AcquisitionCoordinatorPort | One logical owner's acquisition correlation, admitted origin/scope/profile/registration, repair relation, one-use completion and terminal result | Protected-entry/code correlation is consumed once before exchange; current scope checked again before publication. Custody then metadata acknowledgement precede completed. Unknown exchange never grants resend; gateway cannot move/recreate the owner | [acquisition §4](../contracts/auth/acquisition/v1alpha1/semantics.md), [management §3](../contracts/auth/management.md) |
| RefreshCoordinatorPort | Per-source rotating-material alias association, reservation/current fence, authorize-once consumption, stored response and publication/recovery record | Authorize consumes source and invalidates admissions atomically before send. Only the original live acknowledged winner sends once. Lost response/owner is uncertain; acknowledged stored response permits publication-only recovery. No lease or backup can make a consumed source reusable | [acquisition §4.1](../contracts/auth/acquisition/v1alpha1/semantics.md#41-refresh-exclusion-authorization-and-recovery) |
| MaterialAdmissionPort | Immutable capture association and generation evidence/provenance; transient inspected/admitted transport pins | Capture alone grants nothing. Recover verifiable snapshot association or capture/validate anew, subject to refresh alias uniqueness. Final dispatch participates in connection/refresh fencing; missing snapshot or stale evidence refuses without rereading mutable sources | [evidence §§4.1–4.3](../contracts/auth/evidence/v1alpha1/semantics.md) |
| PermissionEvidenceCachePort | Optional bounded exact-target evidence with original collection/deadline and binding coordinates | Cache loss supplies neither denial nor success. Re-collection requires the existing admitted F08 budget or refusal. No independent durable database or permission grant is implied | [evidence §4.4](../contracts/auth/evidence/v1alpha1/semantics.md#44-exact-authorization-targets-and-fan-out-budget-f08) |
| MutationAttemptPort | Durable prepare, dispatch/abort fence and first terminal outcome for one immutable attempt | Prepare grants no send; only the original live acknowledged gate winner sends once. Recovery never sends. Pre-gate abort races atomically with dispatch; unknown gate or terminal persistence keeps conservative recovery knowledge | [operations §4](../contracts/operations/v1alpha1/semantics.md) |
| MutationKeyPort | Exact namespace/key/fingerprint index, reservation-to-attempt link, retained result and settlement/expiry | Reserve plus Prepared attempt is one atomic write; terminal settlement plus replay state/times is another. Pending/Quarantined never expire automatically; known-result expiry is generation-safe. Capacity refuses new reservations, never evicts live deduplication facts | [operations §5.1](../contracts/operations/v1alpha1/semantics.md#51-key-namespace-fingerprint-and-replay-admission) |
| ApprovalSpendPort | Executing leaf's issuer/reference tombstone, canonical subject and one Prepared attempt link; at most one successful redemption per attempt | Acknowledged spend is separate from the dispatch gate. Unknown spend grants no gate/send; spent then aborted stays spent. Direct and gateway ingress share this owner. First-profile tombstones have no automatic expiry; capacity refuses new spending | [delegation §§4.2–6](../contracts/service/delegation.md) |
| DelegationNoncePort | Executing leaf's exact gateway-issuer/receiver/jti receipt | Atomic durable consume precedes entry; ambiguous acknowledgement grants no entry. Recheck current proof/key/policy/time/deadline after acknowledgement. Retire only at trustworthy lower >= exp; uncertain time/restart extends retention | [delegation §5](../contracts/service/delegation.md#5-time-deadlines-and-durable-uniqueness) |
| ExecutionAuditPort | Each admitting hop's durable admission anchor and linked outcome observation, with safe correlation | Unacknowledged admission blocks dispatch. Final append failure preserves known business effect and reports incomplete audit; it does not retry the effect. Gateway and leaf audits are distinct and not one transaction. Required retained audit outlives replay payload deletion | [compatibility §5](../contracts/service/compatibility.md), [delegation §6](../contracts/service/delegation.md#6-end-to-end-admission-and-single-spend) |
| DiscoveryPublicationPort | Exact scope/epoch/selection, attempt/predecessor, immutable classified view and matching private observation index | Publish whole view/index atomically under current source/material/policy/deadline fences. Stale attempt discarded; unknown acknowledgement observed by same attempt. Recover verifiable continuity or invalidate handles/new epoch. Bounded eviction is not disappearance | [resources §4.4](../contracts/discovery/resources/v1alpha1/semantics.md#44-publication-retention-and-public-paging) |
| MediatedRoutePort | Fixed child/parent/observation/target relation, private route evidence revision and current validation | Same-target CAS checks current observation, parent generation, independent admission and child enablement/revocation; invalidates old admissions/verification. Unknown acknowledgement grants no forward. Route change cannot repoint a connection; unavailable private proof/port refuses | [mediated route §4.1](../contracts/discovery/mediated_route/v1alpha1/semantics.md#41-observation-continuity-and-route-revalidation) |
| SessionRedemptionPort | Serving endpoint's one-use establishment authority and redemption decision | Redeem before data; distinct from approval, transport nonce and renewable data lease. A supporting authority binding must prevent reuse across its selected ingress/replica/restart scope. Loss of continuity cannot recreate an accepted establishment token; exact ledger/format remains an advertisement prerequisite | [sessions §4](../contracts/sessions/v1alpha1/semantics.md), [auth capability](../contracts/auth/capability/v1alpha1/semantics.md) |
| SessionSupervisorPort | One live owner's serialized state, first terminal fact, lease/revoke/drain decisions and task/resource accounting | Enforce the existing 2 s data cutoff and 5 s teardown, independently at controlled endpoints. Restart/continuity loss is lost, never reattach or reconstructed live leases. Durable terminal records, if selected, do not restore live sockets/devices or permit token reuse | [sessions §4.1](../contracts/sessions/v1alpha1/semantics.md#41-traffic-cutoff-and-teardown-f06) |
| ReadCursorCachePort | Optional bounded read cache/context/key/expiry; no second discovery view or mutation-result authority | Default ephemeral cursor-key loss invalidates continuations. Discovery cursors refer to DiscoveryPublicationPort's immutable view and cannot retain it past its limits. Cache absence proves no provider absence. Durable subscription checkpoints are a different deferred port | [records](../contracts/datasources/records/v1alpha1/semantics.md), [resources §4.4](../contracts/discovery/resources/v1alpha1/semantics.md#44-publication-retention-and-public-paging) |
| DeploymentConfigurationPort | Authoritative admitted instance/operation/profile/route/configuration/projection meaning and selected issuer/trust policy inputs | Deployment files/artifacts may supply immutable configuration; no live configuration database is implied. Effective connection facts belong to ConnectionAuthorityPort. A configuration backup cannot reset another port's uniqueness or revive old identities, target bindings, grants or private history | [§18](#18-the-connectors-owned-ess-adapter-specification-kind), [delegation §§2,5](../contracts/service/delegation.md) |
| EventClaimSpendPort — unselected | Conceptual one-shot event-derived claim, separate from event receipt and mutation attempt | Exact claim ingress/proof remains unbound. An event or its persisted receipt is not approval. This named future owner cannot advertise spending until its dedicated proof/issuer/ingress contract is settled | [operations §4](../contracts/operations/v1alpha1/semantics.md), [delegation §8](../contracts/service/delegation.md#8-ess-and-verification-boundary) |

### 31.1 Atomic groups and ordered handoffs

**Binding metadata group.** ConnectionAuthorityPort, RefreshCoordinatorPort and the relevant MaterialAdmissionPort decisions must share one linearizable metadata authority for source reservation/alias exclusion, exchange authorization, current generation, private publication fence, terminal revoke and final dispatch ordering. AcquisitionCoordinatorPort completion/publication must participate in that same binding fence. DiscoveryPublicationPort and MediatedRoutePort require atomic comparison against the relevant current connection/generation/revocation and observation/route facts at publication/revalidation. A composition supporting mediation must supply these coupled decisions through that same logical metadata authority or a separately specified equivalent protocol that proves every required comparison; no such alternative protocol is selected here. Separate trait names do not license disconnected stores. Transient pins remain transient; ordering their dispatch is not a claim that every pin is persisted.

**Mutation metadata group.** MutationKeyPort and MutationAttemptPort share atomic reserve/prepare, known terminal settlement/replay-state/timestamp, recovery quarantine and generation-safe retirement. A separately acknowledged ApprovalSpendPort operation precedes the attempt's separate dispatch gate; they need not be one cross-store transaction. An interruption between them deliberately leaves a spent approval and an aborted/uncertain attempt under F01/F03. The spend owner must atomically enforce exact issuer/reference uniqueness and one successful spend per Prepared attempt under current proof/authority/time. It binds the verified durable preparation anchor and exact subject, not a caller-supplied attempt pointer. Abort may win around the spend; the separate abort/open fence still prevents an aborted attempt from sending. No atomic spend-plus-provider or spend-plus-dispatch transaction is claimed. The provider call is never part of the metadata transaction.

**Ordered boundaries.** Custody durable write precedes active-reference publication; a loose immutable version is not a committed refresh response. Admission audit acknowledgement precedes dispatch; final audit follows the observed outcome and can fail independently. Delegation nonce consumption precedes authenticated entry and its post-acknowledgement checks. These orderings require explicit acknowledgements and conservative ambiguity handling, not a distributed transaction with custody, provider, audit exporter, gateway or remote policy service. Current external policy/provider evidence is established through the owning admitted protocol; the host serializes its local known-revocation/binding fences, not an invented transaction over remote authorities.

**Failure and recovery.** No unavailable read is absence. No unacknowledged write/consume/spend/gate grants send, entry, publication or reuse. Read back only the owning record under current admission, following its contract; a replayed success is never a fresh send permit. Restoring/failing over a stable service identity requires its entire relevant uniqueness/fencing history: refresh consumption/alias reservations, pending/unknown keyed attempts, approval tombstones, live delegation receipts and private incarnation continuity cannot diverge across active replicas. If continuity cannot be established, refuse the affected profile; do not import a stale backup as an empty new authority. Bounded stores refuse new work at capacity and retain safety facts under their owner-specific rules. Declared non-durable/in-memory bindings cannot advertise restart-safe authority merely by implementing the same method names.

### 31.2 Revisions, retention and modeled scope

Shared immutable artifact provenance is modeled in `ess/domains/artifact_provenance.yaml` under the [artifact declaration rules](../contracts/catalog/v1alpha1/semantics.md#9-ess-declarations-and-optional-catalog-state). Sources and generated Bundles are independently retained declarations; consumers reference them without deletion ownership. Index replacement does not mutate a Bundle or confer execution authority. This additive model preserves existing adapter/operation identities and current strict reader syntax; optional distribution publication remains separately selected work.

Public connection revision and F02/F03 semantic binding revision identify effect-relevant configuration/management meaning. They change for admitted semantic changes, not for same-identity material refresh, evidence recollection or safe same-target route revalidation alone. The **private auth publication fence** instead advances on auth binding publication/revocation/replace and serializes pending admissions; acquisition/refresh's existing expected_binding_revision carrier names this private fence. Credential generation, discovery generation and private route evidence revision are further distinct coordinates. Refresh may invalidate current dispatch/permission evidence while leaving a request's semantic fingerprint unchanged; current admission still governs replay disclosure. A changed destination/identity/operation meaning cannot be hidden by keeping a semantic revision constant. New fixed target/parent/access-mode/owner bindings require the new connection rules already selected by auth.connection.

Do not unify retention windows: custody follows valid-use plus its declared retention, key known results use the fixed 86,400 s settlement window while Pending/Quarantined remain live, approval tombstones have no automatic expiry, delegation receipts retire only after trustworthy expiry, discovery history/cursors have their own 600 s/300 s bounds, and session live authority follows its lease/cutoff rules. Audit compaction, metadata retention and secret GC cannot erase the evidence still needed by another owner's safety predicate. Custody scope/version, private material/generation/route locators and protected acquisition correlation never enter public responses, ordinary logs or metrics; diagnostic correlation uses separately admitted safe ids. Access-controlled internal port state is not a diagnostic export.

The shared ESS root now declares twenty entities. The eleven existing identities and lifecycles remain intact: AdapterSpecification, OperationDeclaration, ServiceConfiguration, CredentialGeneration, DispatchAdmission, RefreshAttempt, AttemptRecord, KeyReservation, DeliveryReceipt, ApprovalRedemption and Session. Nine additive records complete the selected model scope: Source, Bundle, AuthProfile, Connection, Acquisition, CustodyVersion, AuditRecord, DiscoveryCollection and ResourceObservation. Their owning contracts select identities, references, deletion independence, retention and lifecycle causation; actual atomic storage and trusted predicates remain implementation obligations.

Connection retains one optional bounded baseline EvidenceSnapshot for its active generation and an optional fixed MediatedRouteBinding. CredentialSet, ExternalIdentity, CompositionDeclaration, Curation and gateway idempotency.RouteBinding are values, with no independent entity lifecycle. A discovery collection owns its bounded observations; a child route retains historical coordinates without a live observation foreign key. Audit uses a private injective instance/ref identity and at most one final observation; attempt and selected-connection links reference independently retained records. Immutable Sources and Bundles are reusable, and no optional index owns their deletion. DispatchAdmission remains transient, Session models one live supervisor, and CredentialGeneration identifies immutable capture.
