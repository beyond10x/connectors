...
ESS currently declares eleven entities: AdapterSpecification, OperationDeclaration, ServiceConfiguration, CredentialGeneration, DispatchAdmission, RefreshAttempt, AttemptRecord, KeyReservation, DeliveryReceipt, ApprovalRedemption and Session. Their existing identities/relations remain authoritative; an ESS entity does not imply a durable implementation. DispatchAdmission is transient, Session models one supervisor, and CredentialGeneration is immutable capture identity. EvidenceSnapshot and gateway idempotency.RouteBinding are values, not additional persisted owners. Connection, Acquisition, AuthProfile, custody version/CredentialSet, AuditRecord, discovery collection/observation/mediated route, Composition, subscription and assignment entities are not declared. This inventory selects their logical port ownership; it does not invent their lifecycle/delete semantics or claim a completed persistent entity graph. Those typed models/relations must be settled through ESS before entity-bearing implementation decomposition, with unresolved ownership/cardinality explicitly UNMAPPED. No backend or conformance execution is provided by this section.
