...
Provider-specific permission tuples, discovery recognition/partition values and
restart preparation are typed in the [adapter-owned ESS model](spec/ess/system.yaml).
Shared ESS retains only generic facts and references; this model does not implement provider checks or codecs.
