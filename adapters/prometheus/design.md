...
Operation `series.query_range` selects `datasource.series/v1alpha1` profile
`promql-range`, preserving the old `prometheus-query-range` declaration. This slice is read-only.
Native semantics and future fixtures are owned by [promql-range](contracts/series/v1alpha1/semantics.md).
...
|---|---|---|---|
| `prometheus.anonymous` | none / anonymous / none | static_config | http-anonymous, no credential placement |
| `prometheus.bearer` | http_bearer / service_account / app | static_config | http-bearer, pinned material |
| `prometheus.basic` | http_basic / service_account / app | static_config | http-basic, pinned user/password |
| `prometheus.via_parent` | parent / mediated_access / none | static_config route materialization | mediated-http, parent authenticates fixed hop |
...
These configuration outlines require a selected new strict reader. The anonymous form requires the explicit auth_profile selector and forbids credential/registration members, including explicit null. Bearer/basic forms require their configured references; missing or null is invalid, not anonymous. Mediated configuration selects `prometheus.via_parent` and contains the sealed route binding, with no child base URL or credentials. static_config activates these bindings under host admission; it creates no auth.begin session or protected-entry action.
...
Original evidence: `../connectors/providers/prometheus.toml`,
`crates/integration-monitoring/`, `crates/monitoring-model/` and design 08 at
81459ac4. The predecessor `specs/prometheus/` HTTP OpenAPI 3.1 document is
