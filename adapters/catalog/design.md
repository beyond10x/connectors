...
- **Status:** the `provider` realization is implemented as `connectors-catalog-provider` (`src/`), over bundles built by `connectors-build catalog` from `crates/connectors-catalog`; the `catalog` index-service realization is not. Evidence: `docs/evidence/gitlab-sandbox-20260913/README.md`, section *Through the catalog provider*. One adapter crate `adapters/catalog/` (`docs/design.md:212`) building one executable with two realizations: `catalog` (index service) and `provider` (generic engine bound to one bundle). One build-time pipeline in `crates/connectors-spec` and `crates/connectors-build`.
