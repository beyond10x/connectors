...
Document representation and membership vocabulary is typed in the
[adapter-owned ESS model](spec/ess/domains/documents.yaml).
Shared ESS uses generic body/read decisions and opaque adapter representation identifiers.
...
| `confluence-page-get` | 510 | read, `expand=body.storage,version`; storage format body | preserve body capability → `document` profile `confluence-page`, using current v2 ID/space-filtered collection retrieval |
| `confluence-page-create` | 548 | write | preserve → mutation |
| `confluence-comment-add` | 617 | write | preserve → mutation |
| `confluence-page-search` | 708 | read, CQL, cursor, `expand=version,body.storage,space`; account-timezone dates | preserve native CQL and body-capable list → bounded metadata candidate page plus scoped v2 bulk bodies, with per-item and aggregate bounds |
...
| `datasource.records` `document` | `jira-issue`, `confluence-page` | native Jira fields object or Confluence storage string, nullable provider version, exact representation-specific byte count and explicit bounded delivery (`contracts/datasources/records/v1alpha1/semantics.md`) |
...
{ "service": { "$ref": "urn:connectors:config:v1:service" },
  ...
  "adapter": {
    "jira": { "allowed_projects": [{"id":"10000","key":"ENG"}], "enabled_writes": ["jira.issue.comment.add"], "page_limit": 100 },
    "confluence": { "allowed_spaces": [{"id":"42","key":"ENG"}], "enabled_writes": [], "max_body_bytes": 262144 },
...
Confluence detail uses `GET /wiki/api/v2/pages` with exact ID and receiver-owned space IDs, `status=current`, `subtype=page` and `body-format=storage`. A possible versioned cache hit first makes the corresponding scoped metadata read without a body format; current membership/version and current host/result admission remain required, and the original body observation time is preserved. Jira keys use an admitted `fields=project` metadata lookup to resolve canonical ID; detail then uses a bounded ID/project JQL predicate with `reconcileIssues`, `fields=*all` and one result. Numeric IDs can enter that scoped body query directly. Jira has no synthesized global version and no body-cache reuse.

Native CQL body search keeps provider candidate ordering and continuation, then enumerates bounded v2 ID/space-filtered bulk bodies before publishing that source page. Per-item UTF-8 storage prefixes and aggregate output accounting preserve valid framing; Jira fields stay a complete native object or are explicitly omitted with a truthful byte count, never a broken JSON prefix. Details have at most two calls; body search has at most five, an 8 MiB aggregate provider-byte ceiling and one original deadline. A failed body enumeration does not advance the public cursor. These selected response observations do not claim that separate metadata/body calls are atomic against provider moves. Exact payloads, source/response/retained-state limits and cache/cursor rules are normative in the document contract.
...
`connectors.adapter/v2` is the intended GET-operation frontend; handwritten `/v1`-style obligations remain for mutations until that frontend supports their methods and bodies. The old repository-authored OpenAPI 3.1 sources (`specs/jira/incremental-reads.openapi.yaml`, `specs/confluence/incremental-reads.openapi.yaml`) do not establish the new scoped document binding. Author its selected GET sources and public/native payload schemas against the [captured current official evidence](../../docs/evidence/datasource-semantics-20260908/provider-evidence.md), including Confluence v2 PageBulk and Jira reconciled full-field search. No existing adapter schema, mapping or generated source is claimed to implement them. Full vendor-platform ingestion is unnecessary for this selected surface.
...
- Fixture: auth header placement for basic and bearer; paging for `startAt`, `nextPageToken`, CQL cursor; unseen/moved/aliased document IDs, current scoped cache revalidation, native Jira custom fields and exact numbers, string-prefix versus whole-object-omission bounds, body-bearing CQL aggregate limits and failed-page non-advancement; every mutation with approval, replay, and lost-response scenarios; scope refusal.
...
Operation advertisement and invocation use the [extended availability matrix](../../contracts/service/v1alpha2/semantics.md#321-visibility-lookup-and-refusal-precedence-e12): disabled writes are omitted from describe, but an admitted lookup of a bound disabled ID with a current revision is forbidden. Current policy denial precedes existence/key disclosure; an old still-authorized description is stale_description. Readiness and possession of a one-shot approval do not hide otherwise admitted metadata. Legacy projection has its own eligible set and returns not_found for hidden writes.
