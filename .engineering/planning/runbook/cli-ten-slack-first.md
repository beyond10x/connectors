...
## Continuation checkpoint: delivered first batch and remaining reviews

Connectors PR17 merged at4b32397df2cc2f5bd5ee1c5737891163fb750957; full CI34022162900 passed all twelve workspaces, repository checks and four platform builds, with release publication skipped. The wave now retains that main ancestor throughfe0c56c5; all thirteen Git framing planning events were replayed through AEP and imported artifacts/source verified byte for byte. Reviewed OAuth FULL SQLite81fac96 was merged atec987d82. No incomplete schema3 producer has been published from this integration branch.

The priority personal-local-writes-are-usable story is implemented revision33. Its previously reviewed installed5600867e remains an ancestor of published main, and a fresh closure-time check confirms identical SHA2562db5607c8f99da4d7169b7ce1befaa47c07e35529b1ef5b1cd415ac71bcda01c for the installed CLI and both authorized ordinary daemon executables. The later Git framing fix is not in those installed bytes. No live Slack send or other daemon replacement occurred.

Website PR5 merged7600eaf; generated-only gate99/99, CI34025485980 and Atlas portal checks all passed. Planning closurefe275d0 is also published on Website main. Actual production is the existing automatic immutable source-setv2 e68b6b02e92ce94126fc0ecee7540772472e25ad386183305b6f53565ddc94f2, delivered by Atlas34024493096 into organization-root published commitadae2569086580d314a15a3fc5131506fc671920. Its exact provenance bytes match the observed live site and name Connectors4b32397d/producer34023890652/artifact9986420081. The live gate passed37 repository states,26 Pages repositories and52 routes with one Website commit. No manual retained-lock publication was dispatched over newer source levels. A later scheduled34024834663 failed during source resolution/snapshot before publication; its output gives exit1 without a specific cause. Evidence, successful/failed logs and exact provenance are retained in the parent scratch inventories.

Atlas authority remains clean exact remote mainf3b9f99ba8d2b65b823a08fefabcb13e094c349d in managedwt-679b510698e5. Relay PR24 is merged, and its published clean worktreewt-71907c7c6dcb was retired through finish and reviewed exact-id gc; evidence remains outside the removed tree. Website delivery treewt-8f969c790ffd was likewise finished and removed after source publication, retained build-artifact archive and reviewed exact-id gc.

Migration draft PR23 is published atb1b292a, while local60601765 contains all four proposed ADRs0040–0043 and current Atlas main. Updating the existing branch was refused because the pre-push guard rechecks an already published GitHub merge as a new commit. A bounded guard correction is active in Atlaswt-f0cc2831ff5a; only actual destination heads/tags define existing objects and every truly new commit retains exact bot author/committer checks. No empty attestation, hook bypass, protection relaxation or fabricated migration acceptance is used. Independent review, ordinary source delivery and official hook activation remain next.

Rate stage2 sourceb4dcc815 contains81 coordinator-verified scoped files;90735eac adds only independent HTTP-reader test registration. Whole-unit adversary pass1 has run all twelve deciding cases. Its preliminary complete affected counts are root1205pass/2newfail/4existingignored, runtime346pass/2existingignored, CLI130pass, console87pass. The two confirmed introduced mismatches concern URL validation agreement in the v2 wire and schema3 authoring contracts; shipped Slack documents remain valid. Final immutable report and repair assignment are pending at this checkpoint. No production source changed during review.
...
## Schema3 verification parent retained — 2026-09-06

The integration merge retains publication parent 0c69450921ab1794c81dadec915b717a61bf0983 and all its original AEP events in Git history. Its prospective 12 GiB reserve event at900fac0a is the same coordinator policy already recorded here by bac5717a; the existing integration section remains intact. This dated replay note records that convergence instead of replacing the fuller integration runbook with the candidate's older body. The subsequent verification, evidence and lifecycle operations are replayed through AEP. No source, report or earlier interruption is rewritten.
...
## Tested hosted documentation and explicit operation selection — 2026-09-06

Root committed the three changed hosted documentation owners at 9bac5f2f1466bff45404b146965b215fc91acf98; the fourth scoped schema file is unchanged. Served OpenAPI explicitly selects Operation v1/v2/v3 and Connection v1/v2, preserves every predecessor example and all twelve existing documentation test identities, and documents the actual hosted acquisition Unsupported boundary. Five namespaced canonical frame schemas preserve the version-specific constraints. The added projection test checks all 369 existing schema vectors for Operation v2/v3 and Connection v2 against their independent validity oracles.

The first actual documentation deciding run before implementation recorded 0 passes and 2 product failures, with no compiler failure; its ten-member manifest remains c9619c1148ceffaa8ba6f7135aeb47a958437ae3811a68208d50603712374e79. Root then applied the exact three-file implementation to A's runtime tree after checking both preimages and postimages. The five-member source handoff manifest is 267325b45848ff1258412849eeb1bd277691fef03b5b063de4e9bcb14dd5aacf. The actual command `cargo test --locked --offline -p server hosted::tests::docs --no-fail-fast` finished at 2026-09-06T19:57:48Z with 15 passes, 0 failures and 0 ignored cases. Root independently verified all seven evidence members against manifest 7f0235cbda02ac7cab58021924fe87937410dfbbea41f5da958c7d2bc4c591d5, the exact source bytes and counts. This command ran on A's auth protocol/runtime/dependency snapshot plus root's documentation bytes. Root's checkpoint alone does not yet contain the complete auth source, and neither the full auth gate nor whole-wave green is claimed. Its guarded target peaked at 4,384,456,704 bytes without interruption.

To compile B's actual CLI cases against the real one-shot API, root applied only the frozen early carrier in crates/server/src/local.rs, crates/connectors-runtime/src/one_shot.rs and crates/connectors-runtime/tests/one_shot_runtime.rs to B's common 95fad7c70556a7178cf94070fa5a43657ba9f72c base. Root verified all six frozen manifest entries, exact base preimages and received bytes; manifest 6487f9321a45f14e240f7049f27c0fa76e82733173fbf9708f287ddb216de17e and auth-early-one-shot-carrier-client-application.json preserve the handoff. The carrier already had 2 actual server passes and 1 actual public runtime pass. It retains the older API, but contains neither A's later local authentication preflight nor Connection v2 dispatch. These are declared root writes in B's tree. B never edits or commits them as its own implementation. At final assembly root commits B's assigned files, restores only these exact root-applied preimages, and reconciles completed A source while preserving the Git parents.

B's retained rate tests assert literal Operation v2 request and response identities. Default operation v3 therefore needs an explicit supported older selection rather than a silent compatibility change. Root authorizes operation-only `--protocol-version v2|v3`, default v3, within B's existing CLI and test owners. Select the actual corresponding client and one-shot API exactly once; no negotiation, fallback, renewal on authentication-required, automatic resend or replay. Preserve the wire identity when normalizing presentation. Only add `--protocol-version v2` to those retained rate-test command arguments; keep every existing literal v2 assertion and response byte. Add paired actual v2/v3 exchanges and unknown-version refusal before reading input or contacting a socket. Connection remediation remains a separately selected v2 protocol, and existing target precedence is unchanged. ESS owns the existing target boundary; this choice introduces no forwarded setup handler or typed domain entity.

B's first actual bound CLI grammar run recorded 0 passes and 3 product/parser failures, with no compiler errors. Its missing operation flag and expected-JSON parse observations remain in the original command logs. B is implementing the grammar and explicit selector under its current compiler assignment; A receives the next bounded slot only after B explicitly releases it. Full source assembly, ESS citation/projection closure, strict affected gates and whole-auth review remain pending. The completed OAuth final review and the held credential story are unchanged.
