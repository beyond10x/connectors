...
Verdict: **needs revision**. **7 P2 findings**, no P0/P1. This is a bounded review of `contracts-mutation-classification` (F10/E06), `contracts-restart-idempotency` (F11/E08), and `contracts-mutation-visibility` (E12), not a request to implement runtime/schema support.

Baseline `c7cd05aab947d09992e55bc217aef5db3546f4ec` was frozen with exact git-show inputs before review. `source-hashes.json` contains 53 baseline/pinned-old inputs; `supplemental-source-hashes.json` contains four additional pinned old files. Old repository sources are fixed at `81459ac42ddd518d3942f4b079841e9e0ed6efc8`. Paths/line citations below refer to these frozen bytes. No tracked/planning edits, runtime tests or other reviewer outputs were used. Parent owns current vendor verification; the conclusions here distinguish repository behavior from independently proved provider guarantees.

## Findings

### MP-A-01 — P2 — SIP's executable effects fail the shared mutation discriminator
...
### MP-A-02 — P2 — Define SIP effect knowledge independently of ready-session publication
...
### MP-A-03 — P2 — Docker lifecycle operations need individual, bounded idempotency claims
...
### MP-A-04 — P2 — Preserve Kubernetes UID/resourceVersion semantics before selecting idempotency
...
### MP-A-05 — P2 — Docker mutable names are insufficient as stable lifecycle targets
...
### MP-A-06 — P2 — `none` overstates effect occurrence and blurs replay identity
...
### MP-A-07 — P2 — Select one complete visibility/refusal matrix without rewriting legacy behavior
...
## Minimum textual decision coverage

| Area | Distinct cases that must remain visible in the revision |
|---|---|
| Executable metadata | Valid SIP mutation; semantic human visibility only; network read; unknown/unsupported effects; missing required mutation discriminator; declared metadata without current grant/approval |
| SIP outcomes | No gate; known no-effect refusal; possible SIP send/lost response; known external effect before app binding failure; fully ready handoff; later termination or restart; no automatic redial |
| Docker | Start already running; stop already stopped; restart repeated; lifecycle state changed between deliberate invocations; stable full ID versus reassigned name; changed parameters; lost response versus fresh keyless attempt |
| Kubernetes | Original exact UID/version; accepted patch versus rollout completion; loss before/after patch; same original preconditions after unknown outcome; different writer/version; object replaced under same name; changed input under live key; deliberate new input/approval; no automatic re-read/retry |
| Shared idempotency | none/natural still anchored mutation attempts; provider repeat semantics versus host keyed replay; request_id versus resource UID/version; Pending/Quarantined uncertainty; known-result expiry creates newly admitted effect possibility, not a retry guarantee |
| Visibility | Unimplemented/unbound; implemented disabled; policy denied; dependency not ready; current authorized success; old descriptor after disable; guessed disabled/hidden ID; legacy projection hidden mutation; metadata/result disclosure denied before original-key observation |

These are normative/textual checks now. Current effect-classification values and attempt/session lifecycles in ESS do not prove descriptor classification, provider preconditions, real dispatch counts, grants or storage races. Any new value shapes should retain those limits; no new persistent entities, runtime implementation, unsupported source claims or unrelated media-control work are needed for this cluster.
