...
id: review-result:discovery-b-initial-20260908
kind: review-result
status: active
title: Discovery coverage and composition reviewer B initial
relations:
- reviews: story:contracts-discovery-coverage
- reviews: story:contracts-host-composition
revision: 1
---
# Independent reviewer B — discovery coverage and host composition, initial review

Verdict: **needs revision**. Nine P2 findings below identify bounded semantic requirements for F13 and E03. Baseline: 0e4a8c11885e7008b2825cdcd0d2855cd241d9b4. All 33 reviewed inputs are frozen from exact Git objects in snapshot/ and source-hashes.json, including the three relevant story bodies. No runtime tests, tracked/planning changes, external integration calls or other reviewer outputs were used.

The permission-budget prerequisite already prevents unknown authorization from becoming successful data and selects explicit denied-target coverage. That resolves authorization coverage, but does not yet define an authoritative discovery collection or its publication/withdrawal rules. Current Rust implements a closed Page and ordinary provider continuations; the resource-observation store, materialization and mediated capability remain proposals.

## DC-B-01 — withdrawal has no exact coverage/equality scope (P2; F13)
...
## DC-B-02 — cap, incomplete scan and retained-history behavior is unselected (P2; F13)
...
## DC-B-03 — atomic replacement lacks stale-publisher and binding fences (P2; F13)
...
## DC-B-04 — provider scans, public paging and complete are conflated (P2; F13)
...
## DC-B-05 — route generation pinning has no safe same-target revalidation path (P2; F13)
...
## DC-B-06 — stable observation identity and invalidation reasons contradict adapter wording (P2; F13)
...
## DC-B-07 — concrete wiring is assigned to the generic server (P2; E03)
...
## DC-B-08 — colocation and installed support do not yet imply a mediated capability (P2; E03)
...
## DC-B-09 — proposed persistent observations/routes are presented as modeled relations (P2; shared documentation precision)
...
## Minimum textual audit cases

Include: complete {a,b} then cap/denied b/provider failure; fully complete same scope missing b; complete narrower scope never removing wider-scope evidence; allowed empty scope versus all-denied; last public page of incomplete scan; same snapshot paged across refresh/authority changes; old publisher losing to newer/config/revoke; unchanged identity through generation/credential refresh; UID replacement at same name; current proxy denial against retained old observation; route port missing/cross-process/nested parent; one composition executable with no concrete dependencies in generic host or sibling libraries. Separate source/model/value checking from runtime/provider/concurrency execution in evidence.
