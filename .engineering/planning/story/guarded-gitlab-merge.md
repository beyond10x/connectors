...
## Background recovery binding in progress — 2026-09-11

Continue under the active guarded-merge story after verified commit 0bc49610d5c5a1030f8ab05e782a5e63e7ebcb43. Implement periodic host-owned recovery of existing Prepared/Dispatching attempts, including unkeyed attempts and targets whose connection or current configuration is no longer admitted. This operates on the already modeled AttemptRecord, its service-instance/connection references and existing ledger transitions in ess/domains/mutations.yaml; it adds no persisted entity, ownership relation, migration, public result-access grant or new decomposition.

A read-only bounded keyset scan returns existing attempt references and their retained instance/state. Recovery never infers ownership from a snapshot's state. For an existing instance worker, queue at most one bounded maintenance batch on that retained serialized worker so prior native exchanges have ended. If no worker exists (or its thread has definitely exited), hold the pool's worker-creation lock while fencing that instance's exact attempts; this excludes a concurrent new dispatcher without starting a worker or child. Enforce the retained instance again inside the recovery transaction. The owner lifetime lock remains held through kernel exit. Pending/terminal observations cannot reconstruct native capabilities.

The maintenance thread waits between bounded scans, rotates its private transient cursor and closes metadata before any clock I/O. It acquires configured trusted time only when Prepared entries need known settlement, and validates the selected clock again before use. Dispatching quarantine needs no time. Missing time or metadata leaves unresolved entries conservative; a later pass requires a fresh positive pending observation, never an error-triggered retry or a repeated provider effect. No key expiry, history deletion, approval spend, credential read, provider query or implicit adapter restart occurs. Current policy continues to govern any subsequent result disclosure separately.

The original CLI/private wire formats and request deadlines remain unchanged. Keep maintenance batches bounded, coalesce queued maintenance and stop/join the background thread during owner cleanup. Prove read-only scan behavior without migration, pagination across keyed/unkeyed records, exact-instance refusal, uncertainty/time failures, recovery of revoked/removed targets, live-write exclusion and owned shutdown. Extend disposable production CLI acceptance to show durable recovery without resubmitting the original key. Retain failed runs and run the required gate with two Cargo jobs and Rust 1.88. Original-audit reconciliation, all storage acknowledgement crashes, provider sandbox acceptance, remaining GitLab workflows, C14 create/update head-guard semantics and distributable reproducibility remain open unless separately established.

Root remains the sole writer on primary main. No sub-agents, linked tree, publication, deployment or paid governed run are part of this increment. The full provider sequence and existing blockers are unchanged.

## Verified background recovery checkpoint — 2026-09-11

Periodic owner recovery now fences existing Prepared/Dispatching attempts without another invocation of their business key, including unkeyed attempts and revoked or removed targets. The read-only indexed scan rotates across retained instances, visits at most eight and returns at most 64 pending references for one instance after a five-second wait between passes. It closes metadata before qualified-clock acquisition. Recovery runs between jobs on the existing worker, with one queued maintenance batch, or under the worker-creation lock when no live worker exists. Each fence checks the exact retained attempt/instance binding. A busy hint alone never establishes ownership. Shutdown stops and joins maintenance and instance workers.

Known preparation settlement requires the currently selected trusted clock; missing time leaves it pending, while uncertain dispatch can be quarantined without time. Recovery grants no result access, proof spend, credential read, worker/child launch, suppression change, key expiry, history deletion or provider send. A later scheduled pass needs fresh positive pending evidence; an uncertain acknowledgement alone cannot trigger a mutation retry. These are bindings of the existing AttemptRecord and retained instance/connection references, with no new entity, schema, migration, dependency pin, wire format or decomposition.

docs/evidence/gitlab-background-recovery-20260911/README.md retains commands, failed/corrected logs, 180 selected source hashes and exact tested runtime identities. All 11 disposable production CLI journeys plus the inert subprocess fixture driver pass in 460.12 seconds. The new journeys prove live-write exclusion and owner-crash recovery without original-key resubmission, and recovery of an unkeyed revoked/removed target through an owner started by another instance, followed by refusal of result disclosure. The unkeyed producer remains a durable-port fixture using Approval::NotRequired; it does not establish an approved native pre-dispatch crash. Three new ledger cases and a deterministic live-worker/coalescing test pass. The full Rust 1.88 gate passes, including 129 host tests with 19 explicitly ignored, shared/six native ESS roots, generation/drift, Clippy, boundaries and 315 conformance scenarios. Reference verification reports 46 contracts, 99 pages and no drift. Unchanged website inputs retain prior evidence. Restoring the exact acceptance package selections reproduces all tested executable hashes after the gate's different build selections.

The story, initiative and goal remain active. Next review the original-audit recovery semantics and complete the remaining storage-acknowledgement and concurrent failure cases; do not infer general audit reconciliation from the current idempotent final-append contract. Dedicated GitLab sandbox acceptance, C14 create/update atomic-head semantics, other selected GitLab workflows and isolated distributable reproducibility remain open. Full GitLab still precedes Kubernetes including Helm, PostgreSQL, MCP and remaining providers. Root is the sole implementation/planning writer on primary main. No linked tree, release, push, deployment, Atlas registration, paid governed run or new critic panel belongs to this existing-story increment. The driver blocker and pending sandbox input request are unchanged.

## Exact audit acknowledgement recovery in progress — 2026-09-11

Continue from verified main 857569291b1a0601b27ad2bb8b050cdefa022f2f under this active story. contracts/service/audit.md section 3 selects retry of one exact FinalObservation, while general reconciliation is explicitly deferred. Current owner/mutation.rs creates that observation once but calls Store::append once and discards it on failure. Bind the existing permitted acknowledgement recovery to production finalization: after a failed append, read the exact original audit, acknowledge only a byte-equivalent retained observation, and permit at most one retry of the original observation after definite absence of a final observation. Re-read once after an uncertain retry. Different observations, missing/malformed records and unavailable reads never authorize completion or overwrite.

Retain the UUID, outcome, safe code and timestamp unchanged across this bounded recovery. Extra storage calls check both the original invocation deadline and a 250 ms recovery budget between calls; the existing bounded metadata wait applies within a call. Always preserve the known live mutation/result and the acknowledged audit reference, reporting incomplete if recovery cannot establish completion. No mutation ledger, provider, proof or credential operation may be retried here. Pass the original deadline through both execution and original-result observation finalization.

This uses ess/domains/execution_audit.yaml's existing AuditRecord and FinalObservation, with no new entity, relation, persisted lifecycle, migration, wire shape or dependency. Add deterministic real-SQLite tests for rollback, lost acknowledgement, expired recovery budget, exact-observation conflicts and reconstruction, plus a production-coordinator result-preservation case where feasible. Run affected checks and the full Rust 1.88 gate; retain final CLI regression and artifact identities. General completion of an original audit after a process loses its observation still needs a reviewed persistence binding; this increment does not invent a request-id-to-attempt relation or infer lost outcomes. Existing provider order, sandbox/head-guard blockers, remaining fault matrix and reproducible-distribution requirements remain intact. Root remains the single writer on primary main; no new decomposition or sub-agents.

## Gate timeout-cause finding — 2026-09-11

The first full gate stopped in the existing private write test native_outcomes_and_lost_replies_never_repeat_a_send: its timeout case returned Unknown/Unavailable instead of Unknown/Timeout. All 35 finalizer combinations passed. Source review shows the child can reach its captured wall/monotonic deadline and close before the parent's socket timer fires. PreparedInvocation::commit currently reports the channel's Unavailable without rechecking its already-expired original wall deadline. writes::remaining already defines either original deadline as exhaustion; this is a cause-projection gap, not evidence that a business effect is known.

Correct the existing commit error path to recheck both original deadlines only for an unavailable transport. Keep early loss Unavailable, all other errors unchanged and every possible-write effect Unknown. Add an independent peer fixture that closes at the captured wall deadline and retain the existing strict timeout assertion, effect count and exact child cleanup. No deadline extension, new protocol shape, model entity or provider resend is introduced. Re-run affected private write tests, the changed production CLI regression and full gate; retain the failed gate and first runtime identities separately. Two newly observed active managed trees belong to other work and remain untouched; this root's implementation stays on primary main.

## CLI replay clock-fixture finding — 2026-09-11

The post-timeout-fix full gate passed. The subsequent full production CLI suite returned 11 passed and one failed in 444.70 seconds: guarded_merge.rs:700 observed six clock requests instead of five after owner replay. The preceding provider-count assertion passed. This is not evidence of an extra GitLab send.

Source inspection found that the CLI clock fixture signs the same fixed midpoint on every exchange. The mutation ledger correctly rejects a later lower bound behind its retained lower bound. After an earlier terminal result extrapolates its fixed sample, a new fixed sample can therefore prevent abort settlement of an approval-replayed preparation. The background recovery task may then legitimately acquire another sample, violating this test's assumption that no other pending work exists. Correct the test-only clock to advance synthetic time by elapsed monotonic whole seconds plus one tick per received request, and require the approval-replayed refusal to settle without an AttemptStore failure. Preserve the strict replay clock count, provider count and exactly-one-PUT assertions; include the provider mode in failure diagnostics. The clock remains synthetic and makes no UTC or sandbox claim. No production time rule or replay behavior changes for this fixture finding. Verify the strengthened four-mode journey and full CLI suite, retain the failed run, and recheck affected gate inputs.

## Verified exact audit acknowledgement recovery — 2026-09-11

The production finalizer retains one exact final-observation UUID, outcome, safe code and timestamp. After a failed append acknowledgement it reads the original audit; only an identical retained final observation proves completion, and only a readable original without a final observation permits one unchanged append retry. Additional calls check the original invocation deadline and a 250 ms recovery budget between storage calls. Unresolved failure preserves the acknowledged audit reference and known live business result with incomplete audit. No provider, approval, credential or dispatch operation is retried. Existing AuditRecord/FinalObservation semantics own this binding; no entity, migration, relation, wire shape or dependency pin was added.

Real-SQLite audit tests pass 17 cases with one inert entry ignored. The production finalizer covers 35 constructed-result/fault/budget combinations, including Applied with unusable native output. The final full Rust 1.88 gate passes, including 135 host tests and 315 conformance scenarios. All 11 production CLI journeys plus the inert subprocess entry pass in 464.32 seconds. Reference drift passes for 46 selected contracts and 99 pages; unchanged website inputs retain earlier evidence. Five runtime artifact and 184 selected input hashes match after acceptance. docs/evidence/gitlab-audit-ack-recovery-20260911/README.md retains exact inputs, commands, all results and limitations.

Two failures are retained with their corrections: the first gate exposed deadline/EOF cause projection (possible-write remains Unknown, expired original deadline now reports Timeout); the first post-fix CLI run exposed the test clock's repeated fixed epoch. The synthetic clock now advances across exchanges, and the stronger replay test requires the reused-proof preparation to settle without AttemptStore failure. Strict provider, effect and replay-clock assertions remain. Fault instrumentation stays test-only. The full CLI suite is regression evidence; the constructed finalizer faults do not claim injected native CLI audit failures.
