...
## Current admission remains distinct from session expiry — 2026-09-06

Actual local v3, local Connection v2 and registry checks passed 1, 1 and 2 cases respectively. The first hosted Connection v2 deciding run produced 0 passes and 2 failures: bound Start returned 422 instead of the expected neutral 403, and ordinary v2 returned 400 instead of 200. These are retained batch8 observations under manifest a03d7a171f0df37050e2bf9268a8a0fac4d458b5599b16a6354400c66266f1b4, without a completed hosted-route claim. A released the compiler before B's final client package command; root keeps serial ownership explicit.

A's source inspection identified a separate expiry projection gap: PersonalAuthority::recheck correctly refuses an expired binding, but bound_status calls it before it can return the authorized caller's Expired status or typed Conflict acknowledgement required by Design 22. This is an implementation-support observation, not a completed adversary pass. The concrete expiry-status-proposal.md has SHA256 ac5b78b24dc620470b6d5dc509235be39442d72ae3e507d8df3ec80b840acbf3; root read the complete proposal and its callers before accepting it.

Authorize one additive RemediationAuthority::recheck_status method in the already assigned crates/service/src/remediation.rs, with the existing strict recheck as its default implementation at the actual current clock. It is authority to observe retained status only. Current principal, owner, operation, target, grant and policy checks still apply; implementations may distinguish session expiry from current policy admission. No arbitrary Conflict or Refused result may be ignored and no earlier clock value fabricated. Existing custom authority implementations remain strict by default.
...
## Root full-gate catalog correction — 2026-09-06

The complete root workspace run on 99328258395255dd10038104c1487a64fd5abd8f returned exit 101 with 1,301 passes, two failures and four existing ignores. The original output remains in stage2-runtime-implementation/common-full-gates/root-tests-first.*. Root owns the newly observed crates/catalog-cli/tests/offline_binary.rs correction; the runtime implementor owns the separate preserved hosted protocol-refusal regression in its existing operation_transport.rs scope. Neither is an adversary result.
