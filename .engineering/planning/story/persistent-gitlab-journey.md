...
## 2026-09-10 retained-material revalidation binding

Continue this active story with explicit connections revalidate before the remaining management cases and Kubernetes/PostgreSQL. The existing Connection baseline_evidence and EvidenceSnapshot own same-generation recollection; no new entity, custody version or acquisition is introduced. contracts/auth/evidence/v1alpha1/semantics.md now disambiguates a fresh capture from creation of a new generation, consistent with the already reviewed connection contract. contracts/cli/v1alpha1/owner.md binds a one-use, 30-second retained-material validation admission to the existing transient-use retention/fence records. ess/domains/cli.yaml models the private admission and public input/result; pinned ESS validation passed before runtime edits.

Revalidation admits the current semantic revision/profile policy before owner startup, reads only the exact qualified retained version, and validates through the native adapter. Publication preserves identity/generation/version/public revision, replaces baseline evidence, and advances the private fence. Concurrent repair/revoke/revalidation and known invalidity win over stale results. Positive invalidity of retained material cuts off use; transient failure preserves a still-valid baseline. No implicit business-call probe or credential write is admitted. Lost acknowledgement is observed through connection status without replay.

Verify registry expiry/concurrency/invalidity/unknown-acknowledgement cases and an actual generated-parser CLI fixture after real evidence expiry, with no credential source remaining, then the required gate and affected website/documentation checks. This is still a local fixture milestone; the dedicated GitLab sandbox and paid-driver protocol-loading blockers remain unchanged. One initiative child means no decomposition critic panel is required.

## 2026-09-10 saved-material revalidation checkpoint

Production connections revalidate now accepts the exact connection/public revision and uses the saved qualified custody version without a protected source. It admits the native profile before startup, captures a one-use retained-material guard, dispatches the declared validation within the original 30-second budget, then replaces same-generation baseline evidence under the current fence. Identity, public revision and custody version remain unchanged. Concurrent repair/revoke/recollection, known invalidity and expiry defeat stale results; successful recollection cuts pending admissions. No credential write or implicit business-call revalidation is introduced.

The final repository gate passes with Rust 1.88, ESS, generation, conformance, workspace tests/Clippy and dependency boundaries. Shared ESS compiles 390 declarations. Four new registry tests cover expiry, unchanged exact version, known-expiry preservation, lost acknowledgement/restart, replay, deadlines, competing publication/repair/revoke and invalidity. All 31 local host tests and all three production GitLab CLI journeys pass, including real 60-second expiry followed by source-free revalidation/read, transient 503 preservation and positive 401 cutoff. Website build/typecheck and documentation drift checks pass. Two independent CLI generations reproduce all ten artifacts. Exact commands, logs, hashes and limits are retained in docs/evidence/gitlab-revalidation-20260910/README.md.

The initial full gate found the outdated fifteen-command conformance inventory; it now covers the sixteenth command and confirms that revalidation reads no protected source. An initial registry assertion was corrected to check the existing earlier repair-consumption fence. Final evidence follows both corrections. Native provider semantics, dependency pins, vendor inputs and SQLite migrations remain unchanged; no new entity or decomposition was introduced. The initiative still has one decomposing child, so planning skill section 7 requires no critic panel for this evidence update.
