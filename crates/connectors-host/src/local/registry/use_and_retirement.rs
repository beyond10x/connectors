...
impl Registry {
    pub fn capture_read(
        ...
        self.transaction(now, false, |tx, authority| {
            ...
            if !row.public || row.binding.instance_id!=binding.instance_id || row.binding.adapter_id!=binding.adapter_id { return Err(Failure::NotFound); }
            if row.state=="revoked" { return Err(Failure::Revoked); }
            if row.binding!=*binding { return Err(Failure::Conflict); }
            // This step relies on bounded custody evidence; actual material is
            // resolved next. Missing/outage is distinct, and final dispatch still
            // checks the current generation after the material has been pinned.
            ...
            if !required_scopes.is_empty() && !baseline.granted_scopes.as_ref().is_some_and(|s| required_scopes.is_subset(s)) { return Err(Failure::InsufficientScope); }
            ...
            Ok(ReadUse {id,version:custody_version(authority,&row.scope_id,version_id)?})
        })
    }
