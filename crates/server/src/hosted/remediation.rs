...
pub(crate) fn validated_metadata(
    ...
    if metadata.operation.id() != canonical.id()
        ...
        || metadata.connection.id() != target.endpoint_ref
...
pub(super) fn hosted_bound_refusal(
    ...
    principal: &HostedPrincipal,
    owner: &PrincipalContext,
    request: &protocol::endpoint_v2::EndpointRequest,
) -> RemediationError {
    use protocol::endpoint_v2::EndpointRequest;
    if state.authority.require_remediation_store().is_err() {
        return RemediationError::Unavailable;
    }
    let EndpointRequest::RemediationStart(start) = request else {
    ...
    let metadata_digest = canonical_input_digest(&serde_json::json!({
        ...
        "endpoint_ref": metadata.connection.id(),
    ...
    let acquisition = protocol::endpoint::ConnectSessionCreateRequest {
        integration_ref: metadata.integration_ref.clone(),
