...
use service::{
    ...
    EgressTransportError, EgressTransportFailure, PlanningEnvironment, PrincipalContext,
};
...
use monitoring_model::{
    ...
    validate_input, DISCOVERY_REF, GRAFANA, GRAFANA_DASHBOARDS_LIST, GRAFANA_DATASOURCES_LIST,
};
...
/// One bounded upstream page of the dashboards listing (S-066). The live namespace serves
/// 4,615,274 bytes whole when unbounded — 17.6× the 256 KiB result bound — over at most 1000
/// dashboards (Grafana still served everything at `limit=1000`), so the mean raw object is
/// ≥ 4.6 KiB and a page of five stays under the bound even at ten times that mean. Five is
/// also the page size the live diagnosis verified to page with a `continue` token.
const DASHBOARD_PAGE_LIMIT: usize = 5;
/// Upstream fetches one dashboards dispatch may spend before answering a short page with a
/// cursor — the honest answer when the budget runs out first (S-063's walk). Each page is
/// capped at `MAX_RESULT_BYTES` by the egress bound, so the raw aggregate stays ≤ 5 MiB.
const MAX_DASHBOARD_FETCHES: usize = 20;
/// Dashboards returned when the caller names no `limit`: the fetch budget's full yield.
const DASHBOARD_LIST_DEFAULT_LIMIT: u64 = 100;
    ...
    async fn execute(
        ...
        let response = self
            ...
            .map_err(|error| match error {
                EgressTransportError::ResponseTooLarge => UpstreamFailure::ResponseTooLarge,
                EgressTransportError::Transport(class) => UpstreamFailure::Transport(class),
                EgressTransportError::Refused => {
                    UpstreamFailure::Transport(EgressTransportFailure::Other)
                }
            })?;
...
impl MonitoringInner {
    ...
    async fn invoke(
        ...
        let (result, parent_ref, adapter) = if provider == GRAFANA {
            ...
            let token = self
                ...
                .await?;
            let value = if request.operation_ref == GRAFANA_DASHBOARDS_LIST {
                self.list_dashboards_direct(context, &parent, &token, operation, &request.input)
                    .await?
            } else {
                self.execute_direct(context, &parent, &token, operation, request.input)
                    .await?
            };
    ...
    /// The dashboards listing, walked in bounded upstream pages (S-066). Grafana serves the
    /// whole namespace when the dispatch names no `limit` — 4.6 MB live against the 256 KiB
    /// response bound — so every upstream fetch carries a small page `limit` and the walk
    /// follows `continue` server-side: bounded loop, aggregate, and a short page with a cursor
    /// when the fetch budget runs out first, mirroring the hosted Kubernetes list walk (S-063).
    /// The aggregate stays the raw list shape so the one projection keeps owning redaction.
    async fn list_dashboards_direct(
        ...
        let requested = input.as_object().ok_or_else(operation_invalid)?;
        ...
        let limit = requested
            ...
            .unwrap_or(DASHBOARD_LIST_DEFAULT_LIMIT);
        let mut cursor = requested
            .get("continue")
            .and_then(Value::as_str)
            .map(str::to_owned);
        let mut items = Vec::new();
        for _ in 0..MAX_DASHBOARD_FETCHES {
            ...
            page.insert("namespace".to_owned(), namespace.clone());
            page.insert(
                "limit".to_owned(),
                Value::from(remaining.min(DASHBOARD_PAGE_LIMIT) as u64),
            );
            if let Some(continuation) = cursor.as_deref() {
                page.insert("continue".to_owned(), Value::from(continuation));
            }
            let value = self
                .execute_direct(context, parent, token, operation, Value::Object(page))
                .await?;
            let list = value.as_object().ok_or_else(operation_invalid)?;
            items.extend(
                list.get("items")
        ...
        Ok(serde_json::json!({"items": items, "metadata": {"continue": cursor}}))
    }
