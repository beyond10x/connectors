...
mod tests {
    ...
    #[tokio::test]
    async fn rate_adversary_slack_zero_delay_settles_each_explicit_write_refusal() {
        let root = tempfile::tempdir().unwrap();
        fs::set_permissions(root.path(), fs::Permissions::from_mode(0o700)).unwrap();
        let egress = Arc::new(RateEgress {
            retry: Some("0"),
        ...
        let audit = fs::read_to_string(root.path().join("slack-operation-audit.jsonl")).unwrap();
        let outcomes: Vec<_> = audit
            .lines()
            .map(|line| {
                serde_json::from_str::<serde_json::Value>(line).unwrap()["event"]["outcome"]
            ...
            .collect();
        assert_eq!(outcomes, ["attempted", "refused", "attempted", "refused"]);
        assert!(!audit.contains(SENTINEL));
        backend.shutdown().await;
    }
