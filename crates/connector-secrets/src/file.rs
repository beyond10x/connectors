...
pub(crate) fn validate_transactional_bounds(
    ...
    // The acknowledged-committed form is the longest record grammar. Reserve one full line per terminal record,
    ...
    const MAX_TRANSACTION_LINE_BYTES: usize = 176;
...
impl<L: Layout + Send + Sync> SecretStore for FileStore<L> {
    ...
    async fn put(&self, reference: &CredentialRef, secret: &Secret) -> Result<(), StoreError> {
        ...
        if transactions.prepared().is_some() || transactions.acknowledgement_uncertain {
