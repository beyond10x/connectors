...
#[derive(Parser)]
#[command(about = "Reproduce or check ConnectorOperation v0alpha3 bundle artifacts")]
struct Args {
    #[command(subcommand)]
    mode: Mode,
}
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let prefix = "contracts/connector-operation/v0alpha3";
    ...
    for vector in &vectors.cases {
        ...
        let rust_valid = match vector.kind.as_str() {
            "request" => serde_json::from_slice::<protocol::operation::v3::RequestEnvelope>(&bytes)
                .is_ok_and(|frame| frame.validate().is_ok()),
            "response" => {
                serde_json::from_slice::<protocol::operation::v3::ResponseEnvelope>(&bytes)
                    .is_ok_and(|frame| frame.validate().is_ok())
            }
    ...
    let manifest = pretty(
        &json!({"bundle":"b10x.connector-operation", "version":"v0alpha3",
