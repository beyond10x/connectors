...
pub use domain::endpoint_inventory::{
    EndpointInventoryEntry, EndpointBinding, EndpointCredentialReference, EndpointScheme, EndpointReadiness,
...
pub struct RequestEnvelope {
    ...
    pub request: EndpointInventoryRequest,
}
...
pub enum EndpointInventoryRequest {
    List(ListRequest),
    Show(ShowRequest),
...
pub enum EndpointInventoryResult {
    List {
...
pub enum EndpointInventoryErrorCode {
    NotFound,
    NotGranted,
    InvalidInput,
    Conflict,
...
#[error("{code:?}: {message}")]
#[serde(deny_unknown_fields)]
pub struct EndpointInventoryError {
    pub code: EndpointInventoryErrorCode,
    pub message: String,
    pub retriable: bool,
}

impl EndpointInventoryError {
    pub fn new(code: EndpointInventoryErrorCode, message: impl Into<String>, retriable: bool) -> Self {
        Self {
            code,
...
pub struct ResponseEnvelope {
    ...
    pub response: Option<EndpointInventoryResult>,
    pub error: Option<EndpointInventoryError>,
}

impl RequestEnvelope {
    pub fn validate(&self) -> Result<(), EndpointInventoryError> {
        ...
        let valid = match &self.request {
            EndpointInventoryRequest::List(value) => {
                value
                    .source_ref
                    .as_deref()
            ...
            EndpointInventoryRequest::Show(value) => reference(&value.endpoint_ref, 512),
            EndpointInventoryRequest::Refresh(value) => value
                .source_ref
                .as_deref()
                .is_none_or(|v| reference(v, 512)),
            EndpointInventoryRequest::Bind(value) => {
        ...
        if !valid || serde_json::to_vec(self).map_or(true, |v| v.len() > MAX_FRAME_BYTES) {
            return Err(EndpointInventoryError::new(
                EndpointInventoryErrorCode::InvalidInput,
...
impl ResponseEnvelope {
    pub fn success(request_id: impl Into<String>, response: EndpointInventoryResult) -> Self {
        Self {
    ...
    pub fn failure(request_id: impl Into<String>, error: EndpointInventoryError) -> Self {
        Self {
            protocol: CONTRACT.into(),
            request_id: request_id.into(),
        ...
        match (&self.status, &self.response, &self.error) {
            (operation::ResponseStatus::Ok, Some(result), None) => {
                let valid = match result {
                    EndpointInventoryResult::List {
                        endpoints,
                        next_cursor,
                        warnings,
                    } => {
                        endpoints.len() <= usize::from(MAX_RESULTS)
                            && endpoints.iter().all(EndpointInventoryEntry::validate)
                            && next_cursor.as_deref().is_none_or(|v| reference(v, 4096))
                            && valid_warnings(warnings)
                    }
                    EndpointInventoryResult::Show { endpoint } | EndpointInventoryResult::Bind { endpoint } => {
                        endpoint.validate()
                    }
                    EndpointInventoryResult::Refresh { warnings, .. } => valid_warnings(warnings),
                };
                if !valid {
                    return Err(refusal());
                }
    ...
    pub fn matches_request(&self, request: &EndpointInventoryRequest) -> bool {
        match (&self.response, request) {
            (None, _) => self.error.is_some(),
            (Some(EndpointInventoryResult::List { endpoints, .. }), EndpointInventoryRequest::List(request)) => {
            ...
            (Some(EndpointInventoryResult::Show { endpoint }), EndpointInventoryRequest::Show(request)) => {
                endpoint.endpoint_ref == request.endpoint_ref
            }
            (Some(EndpointInventoryResult::Bind { endpoint }), EndpointInventoryRequest::Bind(request)) => {
                endpoint.endpoint_ref == request.endpoint_ref
                    && endpoint.binding.as_ref() == Some(&request.binding)
            }
            (Some(EndpointInventoryResult::Refresh { .. }), EndpointInventoryRequest::Refresh(_)) => true,
            _ => false,
        }
    }
}
...
fn refusal() -> EndpointInventoryError {
...
mod tests {
    ...
    fn envelope(request: EndpointInventoryRequest) -> RequestEnvelope {
        RequestEnvelope {
            protocol: CONTRACT.into(),
    ...
    fn endpoint_page_and_source_reference_bounds_are_enforced() {
        let mut request = envelope(EndpointInventoryRequest::List(ListRequest {
            source_ref: None,
            query: String::new(),
            limit: MAX_RESULTS,
            cursor: None,
        ...
        let EndpointInventoryRequest::List(list) = &mut request.request else {
            panic!()
        };
        list.limit += 1;
        assert!(request.validate().is_err());
        assert!(envelope(EndpointInventoryRequest::Show(ShowRequest {
