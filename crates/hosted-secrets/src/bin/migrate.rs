...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if metadata.len() > MAX_CONFIG_BYTES {
        return Err("migration configuration is too large".into());
    }
    let config: MigrationConfig = toml::from_str(&fs::read_to_string(&args.config)?)?;
    if config.scopes.is_empty() || config.scopes.len() > 128 {
        return Err("migration needs 1..=128 scopes".into());
    }
    ...
    for scope in config.scopes {
        let scope = CredentialScope::new(&scope.tenant, &scope.authority)
            .map_err(|_| "migration scope is invalid")?;
        for reference in source.references(&scope).await? {
            if destination.exists(&reference).await? {
                existing += 1;
                continue;
            }
            let value = source.get(&reference).await?;
            if let Some(owner) = config.owner_subject.as_deref() {
                destination.put_owned(&reference, owner, &value).await?;
            } else {
                destination.put(&reference, &value).await?;
            }
            copied += 1;
        }
    }
