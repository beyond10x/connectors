...
//! keeps its own list of Endpoints — and everything below `open_inner` is shared.
...
impl GitlabBackend {
    ...
    /// may run as several replicas and they must agree on which Endpoints exist. Which backend
    ...
    /// keeps its own list of Endpoints: one process on one machine writes owner-only files beside
