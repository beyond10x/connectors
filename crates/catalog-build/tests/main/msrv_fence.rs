...
//!
//! `[workspace.package] rust-version` is inherited by every crate here. Nothing publishes, so the
//! promise is not to a downstream consumer — it is to whoever builds this repository on the
//! toolchain it advertises, and a declared minimum nothing checks is a number that drifts silently.
//!
//! The predecessor learned this the expensive way: a caret requirement `jsonwebtoken = "10.3"`
//! resolved to 10.4.0, which declares `rust-version = 1.88.0` against a workspace declaring 1.87,
//! and the incompatibility surfaced only because a person read the lockfile. `resolver = "2"`
//! performs no MSRV-aware version selection, so cargo picked the newest semver-compatible release
//! and said nothing. `resolver = "3"` (root manifest) turns that class of drift into a resolution
//! outcome; this fence is what **fails** on the case the resolver only warns about.
//!
...
///
/// Stated per workspace member rather than against one global number: every crate here inherits the
/// workspace value today, and stating it per member is what keeps the fence correct on the day one
/// of them overrides it.
#[test]
...
/// **The declared MSRV is untested, and this is the test that says so.**
///
/// The fence above reads *declarations*; it cannot see a 1.88 language feature used without a
/// `rust-version` bump, here or in a dependency. Proving the declared minimum builds needs a build
...
fn the_running_toolchain_meets_the_declared_msrv() {
    ...
    let output = Command::new(env!("CARGO"))
        .arg("--version")
        .output()
        .expect("cargo reports its own version");
