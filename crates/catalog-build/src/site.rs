...
///
/// The operations are walked in the IR's own declaration order, which is the order the published
/// document carries them in. Every derived fact — which operation a name denotes, which host the
/// call reaches, which credentials authenticate it — comes from [`crate::surface`], the module the
/// canonical-document lowering reads too, so the two artifacts cannot answer any of them
/// differently.
pub fn provider_entry(connector: &Connector) -> Result<ProviderEntry> {
    let mut operations = Vec::with_capacity(connector.operations.len());
...
fn operation_entry(connector: &Connector, operation: &Operation, host: String) -> OperationEntry {
    OperationEntry {
        ...
        credentials: surface::credential_mechanisms(connector, operation)
            .into_iter()
...
mod tests {
    ...
    fn rendered() -> Value {
        let entry = provider_entry(&connector()).unwrap();
        serde_json::from_str(&document(vec![entry]).unwrap()).unwrap()
    }

    #[test]
    fn serialization_is_deterministic() {
        let first = document(vec![provider_entry(&connector()).unwrap()]).unwrap();
        let second = document(vec![provider_entry(&connector()).unwrap()]).unwrap();
    ...
    fn a_published_prefix_matches_the_placement_the_catalogue_emits() {
        for scheme in [
            ...
            // The published prefix is the literal text the value is prefixed with, trailing
            // space included — the same string a consumer of the canonical document places.
            let published = scheme_prefix(&scheme);
            let expected = match &scheme {
                AuthScheme::Bearer => "Bearer ",
                AuthScheme::Basic => "Basic ",
                AuthScheme::Header { prefix, .. } => prefix.as_str(),
                other => panic!("no prefix is published for {other:?}"),
            };
            assert_eq!(
                published, expected,
                "the site publishes a prefix the placement axis does not carry"
            );
        }
    }

    /// The host keeps its templating and drops the path — the same answer the canonical document
    /// carries, because it is the same function ([`crate::surface::host_of`]).
    #[test]
    fn the_host_keeps_its_templating() {
        let document = rendered();
        assert_eq!(
