...
pub const SCHEMA_ID: &str =
    "https://github.com/beyond10x/connectors/blob/main/catalog/connector-document-v5.schema.json";

/// The frozen schema-2 identity. Its generated bytes remain available for historical consumers.
pub const SCHEMA_V2_ID: &str =
...
#[derive(Serialize)]
struct DocOAuth2<'a> {
    #[serde(skip_serializing_if = "<[connector_spec::PersonalOAuthAdmission]>::is_empty")]
    personal_flows: &'a [connector_spec::PersonalOAuthAdmission],
    #[serde(skip_serializing_if = "str::is_empty")]
    endpoint: &'a str,
...
/// The frozen schema-2 artifact, emitted unchanged beside its distinct successor.
pub fn schema_v2_text() -> String {
    format!(
        "{}\n",
        serde_json::to_string_pretty(schema_v2()).expect("schema 2 serializes")
    )
}

/// Frozen source-fidelity/rate schema 3, retained beside the personal-acquisition successor.
pub fn schema_v3_text() -> String {
    format!(
        "{}\n",
        serde_json::to_string_pretty(schema_v3()).expect("schema 3 serializes")
    )
}
...
fn doc_auth<'a>(connector: &Connector, method: &'a AuthMethod) -> Result<DocAuth<'a>> {
    ...
    let oauth2 = match &method.oauth2 {
        ...
        Some(spec) => {
            ...
            Some(DocOAuth2 {
                personal_flows: &spec.personal_flows,
...
fn doc_operation<'a>(
    ...
    Ok(DocOperation {
        request_semantics: operation.params.request_semantics.word(),
        id: &operation.id,
...
fn doc_params<'a>(
    ...
    if let Some(schema) = &set.body_schema {
        params.push(DocParam {
            ...
            wire: None,
            description: "",
            required: set.body_required.unwrap_or(true),
            schema,
        });
    }
    Ok(params)
