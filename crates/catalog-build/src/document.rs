...
struct Document<'a> {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    verify: Option<&'a str>,
    /// This provider holds a credential it cannot spend — see `Connector::custody_only`.
    ///
    /// Published rather than merely enforced at load time. A consumer reading the catalog must be
    /// able to tell a provider that *happens* to have no operations today from one whose
    /// declaration forbids ever having any; only the second is safe to hand a credential whose use
...
fn lower<'a>(connector: &'a Connector) -> Result<Document<'a>> {
    ...
    // A custody-only provider has no services, not one implicit empty one. `service_names`
    // synthesises `DEFAULT_SERVICE` for a connector that declares none, which is right for every
    // provider that describes a surface and wrong for the one kind that describes none: the
    // synthesised entry would carry an empty `base_url`, which the document schema refuses.
    let services = if connector.custody_only {
        Vec::new()
    } else {
        connector
            .service_names()
    ...
    Ok(Document {
        ...
        custody_only: connector.custody_only,
        services,
        auth,
...
// The test module lives beside this file rather than inside it. `document.rs` is the largest
// production module in the crate and is waived at its current size pending a section-oriented
// extraction; adding test bytes to that number would report growth the fence exists to catch as
// if it were lowering code. Same move, same reason, as `integration-slack`'s and
// `integration-gitlab`'s backends.
