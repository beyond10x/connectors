...
fn render(spec: &Spec) -> Result<String> {
    ...
    s += "}\npub struct PreparedWrite<B: WriteBindings> { request: Request, bindings: Arc<B>, method: WriteMethod, path: Vec<String>, query: Vec<(&'static str, String)>, body: Value, output_schema: Value }\n";
    ...
    for w in &spec.writes {
        ...
        let method = match m.method.as_str() {
            "post" => "WriteMethod::Post",
            _ => "WriteMethod::Put",
        };
        s += &format!(
            "}});\nOk(PreparedWrite {{ request: Request::{ty}(args, context), bindings, method: {method}, path, query, body, output_schema: op.output_schema.clone() }})\n}},\n"
        );
    }
    s += "_ => Err(Error::new(ErrorCode::NotFound, \"operation is not a generated write\")),\n}\n}\n";
    s += "impl<B: WriteBindings> PreparedWrite<B> {\npub async fn execute(self, capability: Box<dyn AuthenticatedWrite>) -> WriteOutcome<Value> {\nlet segments: Vec<&str> = self.path.iter().map(String::as_str).collect();\nlet response = capability.send_json(self.method, &segments, &self.query, &self.body).await;\nmatch self.request {\n";
    for w in &spec.writes {
        let ty = type_name(&w.operation.id);
        let method = method_name(&w.operation.id);
