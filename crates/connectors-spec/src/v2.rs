...
pub struct Mapping {
    ...
    pub response_prefix_limit: Option<u32>,
...
pub fn import(spec: &Spec, spec_path: &Path) -> Result<(Value, Value)> {
    if spec.mappings.iter().any(|m| {
        m.response_prefix_limit
            .is_some_and(|limit| !(1..=1_048_576).contains(&limit))
    }) {
        return Err(refuse("response prefix limit is outside supported bounds"));
    }
    let path = spec_path
        .parent()
        .unwrap_or(Path::new("."))
...
fn render(spec: &Spec) -> Result<String> {
    ...
    if spec
        .mappings
        .iter()
        .any(|m| m.response_prefix_limit.is_some())
    {
        s += "use connectors_sdk::HttpResponsePrefix;\n";
    }
    s += &format!(
    ...
    for m in &spec.mappings {
        ...
        let response = if m.response_prefix_limit.is_some() {
            "HttpResponsePrefix"
        } else {
            "HttpResponse"
        };
        s += &format!(
            "fn prepare_{method}(&self, input: &{ty}) -> Result<{ty}Context>;\nfn finish_{method}(&self, input: {ty}, context: {ty}Context, response: {response}) -> Result<Value>;\n"
        );
    }
    s += "}\n";
    ...
    for m in &spec.mappings {
        ...
        s += "];\n";
        let get = match m.response_prefix_limit {
            Some(limit) => format!("get_prefix(&segments, &query, {limit})"),
            None => "get(&segments, &query)".to_owned(),
        };
        s += &format!(
            "let response = self.http.{get}.await?;\nself.bindings.finish_{method}(args, context, response)\n}},\n"
        );
    }
